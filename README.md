...
Switch user-level CLI auth between named profiles. One command instead of `logout && login` whenever you flip between accounts on `wrangler`, `stripe`, `vercel`, `gh`, `doctl`, `supabase`, Claude Code, OpenAI Codex, or `aws`. Supports cross-service groups (`authgate use titan` flips them all at once), profile rename, and ships zsh completion automatically.
